package com.sentinelx.retail.web;
...
/**
 * Uniform error bodies for the retail API, mirroring the auth/payment shape:
...
public class ApiExceptionHandler {
    ...
    public ResponseEntity<ApiError> onUnexpected(Exception e) {
        log.error("unexpected retail error correlationId={}", org.slf4j.MDC.get("correlationId"), e);
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(ApiError.of("Internal Server Error", "Unexpected error processing the request"));
    }
}